    ...
    while ( 1 )
    {
        ...
        if ( appData.rtc_lcd_update )
        {
            ...
            if ( appData.adc_lcd_update )
            {
                ...
                LCD_PutChar ( '=' ) ;
                LCD_PutChar ( ' ' ) ;
                float b = (appData.adones-'0') + (appData.adtens-'0')*0.1f + (appData.adhunds-'0')*0.01f;
                b=b*100/3.30f;
                char higher_char = (int)b/10+48;
                char lower_char = (int)b%10+48;
                if(b==0){
                    LED_Off(LED_D3);
                    LED_Off(LED_D4);
                    LED_Off(LED_D5);
                ...
                if(b>=100){
                    ...
                    LCD_PutChar ( '0' ) ;
                    LCD_PutChar ( higher_char ) ;
                }
                LCD_PutChar ( lower_char ) ;
                LCD_PutChar ( '%' ) ;
                LCD_PutChar ( ' ' ) ;
                LCD_PutChar ( ' ' ) ;
                LCD_PutChar ( ' ' ) ;
